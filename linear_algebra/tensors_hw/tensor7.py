...
x = np.array([
    [[0,4,4,-1], [4,6,0,-3], [3,0,-5,2], [-3,-6,4,0]],
    [[2,4,-5,3], [-4,-5,-6,-1], [-3,3,1,-2], [-5,2,3,3]
     ]
])
'''
TENSOR
...
def get_new_sym_element_tensor(idx):
    ...
    return round(
    ((1 / factorial(2)) * sum([get_number_tensor(tensor_idx) for tensor_idx in get_special_permutatinons(idx)])), 2)


def get_special_permutatinons(idx):
    all_permutations = []
    x = list(idx)
    i = list(permutations(x[2] + x[3] , 2))
    for elem_i in i:
        all_permutations.append(x[0] + x[1] + elem_i[0] + elem_i[1] + x[4])
